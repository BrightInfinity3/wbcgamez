/* ============================================================
   Animal Battle Champions - Network Layer (WebSocket client)
   Copied from 30/js/network.js (battle-hardened; do not rewrite).
   Only the relay path (/abc/ws) and the override key (ABC_WS_URL)
   differ from the 30 version.
...
var Network = (function () {
  ...
  // Priority:
  //   1) window.ABC_WS_URL (hard override — set from console / before load)
  //   2) localStorage.ABC_WS_URL (persistent dev override)
  //      (own key, NOT GAME_WS_URL — the /30 game shares this origin in
  //      production and its override must not leak into this game)
  ...
  var PRODUCTION_WS = 'wss://ladybug.up.railway.app/abc/ws';

  function resolveWsUrl() {
    if (typeof window !== 'undefined' && window.ABC_WS_URL) return window.ABC_WS_URL;
    try {
      var stored = window.localStorage && window.localStorage.getItem('ABC_WS_URL');
      if (stored) return stored;
    } catch (e) {}
    var host = location.hostname;
    if (host === 'localhost' || host === '127.0.0.1' || host === '') {
      // Local dev — assume Ladybug Gamez is running on :3001 alongside the
      // static server. Override via localStorage.ABC_WS_URL if needed.
      return 'ws://' + (host || 'localhost') + ':3001/abc/ws';
    }
    if (host.indexOf('ladybug') !== -1) {
      var scheme = (location.protocol === 'https:') ? 'wss:' : 'ws:';
      return scheme + '//' + location.host + '/abc/ws';
    }
