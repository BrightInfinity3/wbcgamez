/* ============================================================
   Laser Stacks - Online Multiplayer Module
   Lobby management, game sync, host/guest coordination.
   Ported from 30/js/online.js — host-authoritative over the shared
   Ladybug-Gamez WebSocket relay. Laser Stacks always plays with
   exactly 4 seats (ring slots 0/4/2/6); there is no dealer.
   ============================================================ */
...
var Online = (function () {
  ...
  // ---- Constants ----
  // 4 players on the 8-slot ring: bottom, top, left, right
  var SEAT_FILL_ORDER = [0, 4, 2, 6];
  var NUM_SEATS = 8;
  var MAX_PLAYERS = 4;
  ...
  function hideHostOnlyUI() {
    var ids = ['btn-online-deal', 'btn-change-host'];
  ...
  function hostGame(username) {
    ...
    return Network.createRoom(username).then(function (code) {
      ...
      // Laser Stacks is always 4 players: pre-fill the table with the
      // host at the bottom seat and three AI seats. The host reassigns
      // seats to joiners (or back to AI) by clicking avatars.
      fillDefaultSeats();
  ...
  // Fill the 4 seats: host-controlled human at the bottom, AI elsewhere.
  function fillDefaultSeats() {
    var animals = SpriteEngine.getAnimalList();
    var used = [];
    for (var i = 0; i < SEAT_FILL_ORDER.length; i++) {
      var idx = SEAT_FILL_ORDER[i];
      var available = animals.filter(function (a) { return used.indexOf(a) === -1; });
      var animal = available[Math.floor(Math.random() * available.length)];
      used.push(animal);
      var isHostSeat = (i === 0);
      lobbyState.seats[idx] = {
        occupied: true,
        animal: animal,
        name: isHostSeat ? (myUsername || 'You') : pickNickname(animal),
        isHuman: isHostSeat,
        isAI: !isHostSeat,
        deviceId: isHostSeat ? myDeviceId : null
      };
    }
  }
  ...
  function startOnlineGame() {
    ...
    // Build the 4-player list from the lobby seats.
    var players = [];
    var id = 0;
    for (var i = 0; i < NUM_SEATS; i++) {
      var seat = lobbyState.seats[i];
      if (!seat.occupied) continue;
      var p = Game.createPlayer(id, i, seat.animal, seat.name, seat.isHuman);
  ...
  function renderOnlineLobby() {
    ...
    if (countEl) countEl.textContent = totalPlayerCount() + '/' + MAX_PLAYERS + ' Players';
    ...
    if (dealBtn) {
      ...
      dealBtn.disabled = totalPlayerCount() !== MAX_PLAYERS;
    }
